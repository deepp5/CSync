    ...
    try{    
        ...
        if (username !== undefined) {
            ...
            if (cleanedUsername.length === 0) {
                ...
                const isValid = /^[a-zA-Z0-9](?:[a-zA-Z0-9_.-]{1,18}[a-zA-Z0-9])?$/.test(cleanedUsername);
                if (!isValid) {
                return res.status(400).json({
                    error: "Invalid username. Use 2–20 characters: letters, numbers, _.- Must start and end with a letter or number."
                });
                }
...
router.put("/password", verifySupabaseToken, async (req, res) => {
  try {
    ...
    if (!userEmail) {
      return res.status(400).json({ error: "Missing user email in auth token" });
    }
